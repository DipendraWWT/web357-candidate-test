...
namespace Webtest\Component\Web357test\Administrator\Model;
// No direct access.
 ...
 * @since  1.0.0
 */
class RecipeModel extends AdminModel
{
	/**
	 ...
	 * @since  1.0.0
	 */
	protected $text_prefix = 'COM_WEB357TEST';
	 ...
	 * @since  1.0.0
	 */
	public $typeAlias = 'com_web357test.recipe';
	...
	public function duplicate(&$pks)
	{
		...
		// Access checks.
		if (!$user->authorise('core.create', 'com_web357test'))
		{
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			{
				$db = $this->getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__web357test_recipes');
				$max             = $db->loadResult();
